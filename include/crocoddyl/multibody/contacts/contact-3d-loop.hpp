...
#ifndef CROCODDYL_MULTIBODY_CONTACTS_CONTACT_3D_LOOP_HPP_
#define CROCODDYL_MULTIBODY_CONTACTS_CONTACT_3D_LOOP_HPP_
...
class ContactModel3DLoopTpl : public ContactModelAbstractTpl<_Scalar> {
 public:
  ...
  typedef ContactData3DLoopTpl<Scalar> Data;
  ...
  /**
   * @brief Initialize the 3d loop-contact model from joint and placements
   *
   *
   ...
   */
  ContactModel3DLoopTpl(boost::shared_ptr<StateMultibody> state,
  ...
  virtual ~ContactModel3DLoopTpl();

  /**
   * @brief Compute the 3d loop-contact Jacobian and drift
   *
   * @param[in] data  3d loop-contact data
  ...
  /**
   * @brief Compute the derivatives of the 3d loop-contact holonomic constraint
   *
   * @param[in] data  3d loop-contact data
   * @param[in] x     State point \f$\mathbf{x}\in\mathbb{R}^{ndx}\f$
   * @param[in] u     Control input \f$\mathbf{u}\in\mathbb{R}^{nu}\f$
  ...
  /**
   * @brief Create the 3d loop-contact data
   */
   ...
   * @param[in] res   Skew-symmetric matrix
   */
  // void SkewMatrix(const Vector3s &v, Matrix3s res);
  void SkewMatrix(
  ...
  /**
   * @brief Print relevant information of the 3D loop-contact model
   *
...
template <typename _Scalar>
struct ContactData3DLoopTpl : public ContactDataAbstractTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  ContactData3DLoopTpl(Model<Scalar> *const model,
                       pinocchio::DataTpl<Scalar> *const data)
      : Base(model, data),
...
/* --- Details -------------------------------------------------------------- */
/* --- Details -------------------------------------------------------------- */
#include "crocoddyl/multibody/contacts/contact-3d-loop.hxx"

#endif  // CROCODDYL_MULTIBODY_CONTACTS_CONTACT_3D_LOOP_HPP_
